...
const EditArticle = ({match}) => {
  ...
  const fetchArticleData = async () => {
    ...
    await axios({
      url: `http://localhost:7000/article/${articleId}`,
      method: 'GET',
    ...
    storageRef.on(
      ...
      () => {
        storageRef.snapshot.ref.getDownloadURL().then(async (downloadURL) => {
          await axios({
            method: "POST",
            url: `http://localhost:7000/admin/editArticle/editCoverImage/${articleId}`,
            data: {
  ...
  const checkPostTitle = () => {
    setPostTitle(articles.articleTitle);
  }
  ...
  const checkPostDesc = () => {
    setPostDesc(articles.articleDiscription);
  }
  ...
  const checkPostUrl = () => {
    setPostUrl(articles.youtubeUrl);
  }
  ...
  return(
    <> 
          ...
          <input type="hidden" ref={visiRef} required value={articles.articleVisibility} className="InputBox" />
          <img src={articles.articleCoverImage} className="articleImage" alt="" />
          <div className="publicPrivateBox">
            <div onClick={changeArticleVisibility} className="acessPointer" style={visibility ? styleToggle : styleToggle2}></div>
            <div className="accessPointerIcon">
              <i class="fas fa-lock"></i>
              <i class="fas fa-lock-open"></i>
            </div>
          </div>
          <input id="imageInput" ref={coverImage} type="file" onChange={changeArticleCoverImage} />
          <label id="imageLabel" className="editArticleImageButton" htmlFor="imageInput">
        ...
        <div className="editWindow">
          <div className="subEditBox">
            ...
            <span></span>
            <input type="text" required value={postTitle || articles.articleTitle} onChange={updateTitleInput} className="InputBox" />
            <button onClick={changeArticleTitle} class="editUpdateButton"><i class="fas fa-cloud-upload-alt"></i> SAVE</button>
          </div>
          <div className="subEditBox">
            <p className="editTitle">
              Change Article Description
            </p>
            <span></span>
            <textarea rows="5" value={postDesc || articles.articleDiscription} onChange={updateDescInput} type="text" required className="textInputBox"></textarea>
            <button onClick={changeArticleDesc} class="editUpdateButton"><i class="fas fa-cloud-upload-alt"></i> SAVE</button>
          </div>
          <div className="subEditBox">
            <p className="editTitle">
              Change YouTube Link
            ...
            <span></span>
            <input value={postUrl || articles.youtubeUrl} onChange={updateUrlInput} type="text" required className="InputBox" />
            <button onClick={changeArticleUrl} class="editUpdateButton"><i class="fas fa-cloud-upload-alt"></i> SAVE</button>
          </div>
        </div>
