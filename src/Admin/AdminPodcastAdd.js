...
const AdminPodcastAdd = () => {
  ...
  const fakeAudioPlayer = useRef();
  const [podAudioDuration, setPodAudioDuration] = useState();
  ...
  //to check whether they had uploaded image and cover
  const [coverImageAdded, setCoverImageAdded] = useState(false);
  const [audioAdded, setAduioAdded] = useState(false);
  ...
  const findAudioLenght = (e) => {
    const audioFiles = e.target.files[0];
    const file = URL.createObjectURL(audioFiles);
    fakeAudioPlayer.current.src = file;
    fakeAudioPlayer.current.load();
    setAduioAdded(true);
  };

  const settingDuration = () => {
    let podcastDuration = calculateTime(fakeAudioPlayer.current.duration);
    setPodAudioDuration(podcastDuration);
  };
  ...
  return (
    <div className="AdminArticlesView_container">
          ...
          <div className="optionalSubBox">
            ...
            <div className="AdminArticle_SubBox">
              ...
              <input
                ...
                ref={podCoverImage}
                id="podCoverImage"
                onChange={changeCoverImageStatus}
              />
      ...
      <audio ref={fakeAudioPlayer} onLoadedMetadata={settingDuration}></audio>
    </div>
  );
};

export default AdminPodcastAdd;
